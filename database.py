...
class Database:
        ...
        async with pool.acquire() as conn:
            ...
            await conn.execute("""
                CREATE TABLE IF NOT EXISTS notifications (
                    ...
                    UNIQUE (user_id, ntype, period_key)
                )
            """)
    ...
    # ── Обмен нал → карта (с комиссией) ─────────────────────────────────────

    async def add_conversion(self, user_id: int, cash_amount: int, fee_percent: float) -> Dict[str, Any]:
        """Считает комиссию, сохраняет конверсию и списывает комиссию как расход."""
        ...
        async with pool.acquire() as conn:
            async with conn.transaction():
                row = await conn.fetchrow("""
                    ...
                    VALUES ($1, $2, $3, $4, $5)
                    RETURNING id, date
                """, user_id, cash_amount, fee_percent, fee_amount, net_amount)
                await conn.execute("""
                    INSERT INTO transactions (user_id, type, category, amount)
                    VALUES ($1, 'expense', 'Комиссия за обмен', $2)
                """, user_id, fee_amount)
            return {
    ...
    async def get_conversions(self, user_id: int, limit: int = 10) -> List[Dict[str, Any]]:
        pool = await get_pool()
        async with pool.acquire() as conn:
            rows = await conn.fetch("""
                SELECT id, cash_amount, fee_percent, fee_amount, net_amount, date
    ...
    async def get_week_expenses(self, user_id: int) -> int:
        pool = await get_pool()
        async with pool.acquire() as conn:
            row = await conn.fetchrow("""
                SELECT COALESCE(SUM(amount), 0) AS total
    ...
    async def has_transaction_today(self, user_id: int) -> bool:
        pool = await get_pool()
        async with pool.acquire() as conn:
            row = await conn.fetchrow("""
                SELECT 1 FROM transactions
                WHERE user_id = $1 AND date::date = NOW()::date
                LIMIT 1
            """, user_id)
    ...
    async def was_notified(self, user_id: int, ntype: str, period_key: str) -> bool:
        pool = await get_pool()
        async with pool.acquire() as conn:
            row = await conn.fetchrow("""
                SELECT 1 FROM notifications
                WHERE user_id = $1 AND ntype = $2 AND period_key = $3
            """, user_id, ntype, period_key)
            return row is not None

    async def mark_notified(self, user_id: int, ntype: str, period_key: str):
        pool = await get_pool()
        async with pool.acquire() as conn:
            await conn.execute("""
                INSERT INTO notifications (user_id, ntype, period_key)
                VALUES ($1, $2, $3)
                ON CONFLICT (user_id, ntype, period_key) DO NOTHING
            """, user_id, ntype, period_key)
